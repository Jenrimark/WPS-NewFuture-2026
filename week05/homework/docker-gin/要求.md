...
本项目重点考察： 前后端分离架构、代理与跨域处理、第三方 AI 接口对接、关系型数据库设计，以及基于 Docker 的全栈工程容器编排部署。
...
配置管理： Viper
⚠️ 【特别要求：跨域处理规范】
本项目严禁在后端 Go 代码中配置任何允许跨域（CORS）的中间件！
...
逻辑流程：
i. 鉴权通过后，后端先在数据库中检查当前用户是否已保存过该单词。
ii. 如果已保存，直接从数据库读取并返回给前端。
iii. 如果未保存，根据 ai_provider 调用对应 AI 接口。
iv. 让 AI 返回格式化的 JSON 数据（包含：释义 + 3 条例句）。
v. 直接将 AI 结果返回给前端展示，此时不在后端进行数据库保存。
...
后端构建 (backend/Dockerfile)： 使用多阶段构建编译 Go 应用，暴露后端端口（如 8080）。镜像需尽可能精简。

前端构建 (frontend/Dockerfile) 与环境配置：
