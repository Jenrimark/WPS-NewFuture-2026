...
router.post('/', authenticateToken, async (ctx) => {
  ...
  if (!name || !student_no) {
    return fail(ctx, 400, '学生姓名和学号不能为空');
  }
  ...
  const normalizedCourseIds = Array.isArray(course_ids)
    ? course_ids.map(Number).filter((id) => Number.isInteger(id) && id > 0)
    : [];
...
router.put('/:id', authenticateToken, async (ctx) => {
  ...
  const finalStudentNo = student_no ?? existing.student_no;
  if (!finalStudentNo) {
    return fail(ctx, 400, '学号不能为空');
  }

  const duplicate = db.prepare('SELECT id FROM students WHERE student_no = ? AND id != ?').get(finalStudentNo, studentId);
  if (duplicate) {
    return fail(ctx, 400, '学号已存在');
  }
  ...
  const normalizedCourseIds = Array.isArray(course_ids)
    ? course_ids.map(Number).filter((id) => Number.isInteger(id) && id > 0)
    : JSON.parse(existing.course_ids || '[]');
  ...
  `).run(
    ...
    status ? (status === 'inactive' ? 'inactive' : 'active') : existing.status,
    JSON.stringify(normalizedCourseIds),
    studentId
  );
