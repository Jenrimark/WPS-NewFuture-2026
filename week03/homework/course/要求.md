...
改用左右分开设计左侧看下面的方案，右侧原来的界面

视觉资产与布局 (Visual & Layout)：

静态元素： 包含背景图属性（滤镜、遮罩、缩放方式）、UI 组件（输入框、按钮）的尺寸、圆角、阴影（Box-shadow）、边框属性及配色方案（十六进制色值）。

资产说明： 明确图片素材的占位符、格式要求及加载优先级。

动态视觉效果 (Motion Effects)：

入场动画： 页面加载时的元素平移、渐变、缩放路径（如 Cubic-bezier 曲线数据）。

背景动态： 粒子效果、微光流动或视频背景的参数定义。

交互逻辑与操作流 (Interaction Logic)：

鼠标/触控操作： 悬停（Hover）、点击（Active）、聚焦（Focus）时的状态变化（如颜色偏移、尺寸微调）。

眼动/视差关联： 描述鼠标移动或移动端陀螺仪触发的“视差滚动”效果（位移系数、阻尼感）。

触控手势： 针对移动端的长按、滑动切换等响应逻辑。

数据与状态定义 (Data & State)：

输入校验： 定义表单数据的实时验证逻辑、错误状态的视觉反馈。

接口对接： 预留登录请求的数据结构字段（Username, Password, Token 处理）。

输出要求：

使用专业前端开发术语（如 CSS 属性名、动画帧定义）。

逻辑严密，消除歧义，确保复现结果“一模一样”。
